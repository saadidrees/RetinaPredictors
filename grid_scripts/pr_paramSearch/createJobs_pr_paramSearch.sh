...
samps_shift_all=( $(tail -n +2 $PARAMS_FILE | cut -d ',' -f8) )
r_sigma_all=( $(tail -n +2 $PARAMS_FILE | cut -d ',' -f9) )
r_phi_all=( $(tail -n +2 $PARAMS_FILE | cut -d ',' -f10) )
r_eta_all=( $(tail -n +2 $PARAMS_FILE | cut -d ',' -f11) )
r_k_all=( $(tail -n +2 $PARAMS_FILE | cut -d ',' -f12) )
r_h_all=( $(tail -n +2 $PARAMS_FILE | cut -d ',' -f13) )
r_beta_all=( $(tail -n +2 $PARAMS_FILE | cut -d ',' -f14) )
r_hillcoef_all=( $(tail -n +2 $PARAMS_FILE | cut -d ',' -f15) )
r_gamma_all=( $(tail -n +2 $PARAMS_FILE | cut -d ',' -f16) )
...
do
 ...
 JOB_ID=$(sbatch --export=LOG_DIR=$LOG_DIR,path_mdl=$path_mdl,trainingDataset=$trainingDataset,testingDataset=$testingDataset,expDate=$expDate,mdl_name=$mdl_name,path_excel=$path_excel,path_perFiles=$path_perFiles,samps_shift=$samps_shift,r_sigma=$r_sigma,r_phi=$r_phi,r_eta=$r_eta,r_k=$r_k,r_h=$r_h,r_beta=$r_beta,r_hillcoef=$r_hillcoef,r_gamma=$r_gamma pr_paramSearch_launcher.sh)

echo $JOB_ID
JOB_ID=$(echo "$JOB_ID" | grep -Eo '[0-9]{1,8}')
 
printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t' $JOB_ID $expDate $path_mdl $trainingDataset $testingDataset $mdl_name $path_excel $path_perFiles $samps_shift,$r_sigma $r_phi $r_eta $r_k $r_h $r_beta $r_hillcoef $r_gamma | paste -sd '\t' >> job_list.csv
