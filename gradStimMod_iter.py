...
Overview:
    The script allows to modify stimuli that affects the firing rate of a group of selected RGCs while keeping the activity unaffected for other types.
    This is done by calculating gradients of RGCs firing rates wrt stimulus. The gradients give the direction of max sensitivity i.e. modifying the stimuli in
    those directions maximise RGC response. The logic through stimulus can be modified to minimize firing of specific RGCs while unaffecting others is as:
        1. Calculate gradients for all the RGCs
        2. To minimize RGC firing, we need to change stimulus in opposite direction to the gradient. But to leave other RGCs unaffected we have to also ensure
            that the direction we move is orthogonal to those RGCs.
        3. So first calculate orthonormal basis for RGCs that are not to be affected. This ensures that whatever we project on those basis is orthogonal to all unaffected RGCs.
            These orthonormal basis are calculated using gram schmidt process
        4. Then select one RGC that is to be minized, project its gradients orthogonal to the orthonormal basis representing unaeffected cells.
        5. Do this for all RGCs to be minimized. Take avergae of the resulting gradients.
        6. Change stimulus by a delta step in opposite direction to the resulting mean gradient projections.
        7. Feed the stimulus back into the model and calculate updated RGC predictions. Repeat above steps until modified stimuli leads to minimal RGC response in selected RGCs. 
...
path_figs = '/home/saad/data/analyses/gradients_mike/figs/0.1/'
...
t_start=120
# Only select 1 trial
X = data_train.X[t_start:,:,:,mov_select,trial_select]
y = np.mean(data_train.y[t_start:,:,mov_select,:],axis=-1)
data_train = Exptdata(X,y)
data_train =  prepare_data_cnn2d(data_train,80,np.arange(data_train.y.shape[1]))

# rate_sameStim_trials = np.moveaxis(data_train.y,-1,0)
...
pred_rate_cnn = mdl_cnn.predict(data_train.X)


# CNN
fev_cnn_allUnits, fracExplainableVar, predCorr_cnn_allUnits, rrCorr_cnn_allUnits = model_evaluate_new(
    data_train.y.copy(),pred_rate_cnn.copy(),0,RR_ONLY=False,lag = 0,obs_noise=obs_noise.copy())
...
def projection_orthogonal_to_basis(A, basis):
    ...
    orthogonal_projection = jnp.nansum(projections, axis=0)
...
stim_orig = np.asarray(data.X)
y_orig = np.asarray(data.y)
stim_modified = stim_orig.copy()
y_pred_orig = mdl_totake.predict(stim_modified)
...
stim_shape = (None,)
grads_chunk_allUnits = np.zeros((len(idx_unitsToExtract),nsamps,temporal_width_grads,data.X[0].shape[-2],data.X[0].shape[-1]),dtype='float32')
...
for epoch in tqdm(range(n_epochs)):
    for i in range(0,total_batches):
        ...
        for u in range(len(idx_unitsToExtract)):
            ...
            grads_chunk_allUnits[u,idx_chunk] = grads_chunk
    ...
    grads_rgc_shut_mean = np.array(jnp.mean(grads_rgc_shut,axis=0))
    ...
    i=0;j=0
    for i in range(grads_rgc_shut_vec.shape[0]):        # iterate over units
        grad_proj_vec = []
    ...
    for i in range(len(rgc_shut)):
        axs[0,i].plot(y_orig[:,rgc_shut[i]],'lightgray');axs[0,i].plot(y_pred_orig[:,rgc_shut[i]]);axs[0,i].plot(y_pred_mod[:,rgc_shut[i]],'--');axs[0,i].set_title(uname_all[rgc_shut[i]])
    ...
    h0 = axs[0].imshow(stim_orig[img_idx,idx_temp_peak],cmap='gray',vmin=vmin_img,vmax=vmax_img);axs[0].set_title('Stim');plt.colorbar(h0,fraction=0.035,pad=0.02)
    ...
    h4=axs[4].imshow(step*grad_toadjust_mean_reshaped[img_idx,idx_temp_peak],cmap='gray');axs[4].set_title('Gradient adjusted');plt.colorbar(h4,fraction=0.035,pad=0.02)
